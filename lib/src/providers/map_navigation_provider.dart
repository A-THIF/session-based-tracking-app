...
class MapNavigationNotifier extends StateNotifier<MapNavigationState> {
  ...
  void _startCompassListener() {
    ...
    _compassSub = FlutterCompass.events!.listen((event) {
      ...
      double target = -rawHeading; // Map rotates opposite to device

      double diff = (target - current + 180) % 360 - 180;
      if (diff < -180) diff += 360;

      // 🌊 Low-pass filter (15% smoothing)
      double smoothedRotation = current + diff * 0.15;

      state = state.copyWith(rotation: smoothedRotation);
    });
  }
...
final mapNavigationProvider =
    StateNotifierProvider<MapNavigationNotifier, MapNavigationState>((ref) {
      return MapNavigationNotifier(ref);
    });
